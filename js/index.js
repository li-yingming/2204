   ...
   getData() {
      ...
      axios.get('http://localhost:3000/problem').then(res => {
         ...
         if (status == 200) {
            ...
            // 4 将获取的数据,渲染到页面中
            /*
               1删除的第一种思路 ,直接给行内绑定事件,但是回调方法要是静态的
                   <button type="button" class="btn btn-danger btn-xs " onclick="Problem.delData()">删除
               ...
               2 使用时间委托,将删除和修改操作的点击事件委托给tbody(用的这种方法)
